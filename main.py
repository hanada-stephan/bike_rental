...
def main():
    ...
    lstm_model.add(LSTM(128, input_shape=(X_train_lag.shape[1], X_train_lag.shape[2])))
    lstm_model.add(Dense(units=1))
    lstm_model.compile(loss="mean_squared_error", optimizer="RMSProp")
    # Fitting the model
    result_lstm = lstm_model.fit(
        X_train_lag, y_train_lag, validation_data=(X_test_lag, y_test_lag), epochs=100
    )
    # Prediction
